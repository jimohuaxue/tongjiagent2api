...
def _candidate_id(value: Any) -> str | None:
    ...
    if isinstance(value, dict):
        for key in (
            ...
            nested = _candidate_id(value.get(key))
            if nested:
                return nested
    return None


def extract_client_conversation_id(raw_body: dict[str, Any]) -> str | None:
    """
    从请求体里提取客户端侧“同一对话”的稳定 ID。

    这个 ID 不是上游网页的真实 session_id，而是调用方传来的会话键；服务端会把它
    映射到真实网页会话。优先支持显式字段，也兼容 metadata/conversation 包装。
    """
    for key in (
        ...
        value = _candidate_id(raw_body.get(key))
        if value:
            return value
    ...
    if isinstance(metadata, dict):
        for key in (
            ...
            value = _candidate_id(metadata.get(key))
            if value:
                return value
    return None
