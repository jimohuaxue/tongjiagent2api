...
from core.usage import UsageRecord, estimate_tokens, record_usage
...
def _select_attachments_for_request(
    req: OpenAIChatRequest,
    *,
    full_history: bool,
) -> tuple[list[Any], str]:
    if req.attachment_files_last_user:
        return req.attachment_files_last_user, "last_user"
    if full_history and req.attachment_files_all_users:
        return req.attachment_files_all_users, "all_users"
    return [], "none"


def _estimate_request_input_tokens(req: OpenAIChatRequest) -> int:
    total = 0
        ...
        if isinstance(msg.content, str):
            ...
            for part in msg.content:
                if part.type == "text" and part.text:
                    ...
                    total += _estimate_file_tokens(part.mime_type or part.media_type or "", part.file_data or part.data or "")
        if msg.tool_calls:
            total += estimate_tokens(json.dumps(msg.tool_calls, ensure_ascii=False))
    for att in req.attachment_files_last_user or req.attachment_files_all_users:
        total += _estimate_attachment_tokens(att.mime_type, len(att.data))
    return total


def _estimate_file_tokens(mime_type: str, data: str) -> int:
    mime = (mime_type or "").split(";", 1)[0].lower()
    if mime.startswith("text/") or mime in {"application/json", "application/xml"}:
        return estimate_tokens(data)
    return max(1, min(200_000, len(data) // 4))


def _estimate_attachment_tokens(mime_type: str, size: int) -> int:
    mime = (mime_type or "").split(";", 1)[0].lower()
    if mime.startswith("image/"):
        return 256
    if mime.startswith("text/"):
        return max(1, size // 4)
    return max(1, min(200_000, size // 4))
...
class ChatHandler:
    ...
    def __init__(
        ...
        self._busy_client_conversations: set[tuple[str, str]] = set()
    ...
    async def _stream_completion(
        ...
        type_name: str,
        req: OpenAIChatRequest,
    ) -> AsyncIterator[_CompletionChunk]:
        """
        ...
        explicit_session_id = (req.resume_session_id or "").strip()
        marker_session_id = parse_conv_uuid_from_messages(raw_messages)
        ...
        client_conversation_key: tuple[str, str] | None = None
        client_conversation_claimed = False
        mapped_session_id: str | None = None
        if client_conversation_id:
            client_conversation_key = (type_name, client_conversation_id)
            async with self._schedule_lock:
                if client_conversation_key in self._busy_client_conversations:
                    raise RuntimeError("当前客户端会话正在处理中，请稍后再试")
                self._busy_client_conversations.add(client_conversation_key)
                client_conversation_claimed = True
                mapped_session_id = self._session_cache.get_by_client_conversation(
                    type_name,
                    client_conversation_id,
                )

        session_candidates: list[str] = []
        for candidate in (explicit_session_id, marker_session_id, mapped_session_id):
            if candidate and candidate not in session_candidates:
                session_candidates.append(candidate)
        logger.info(
            "[chat] type=%s session_candidates=%s client_conversation_id=%s",
            type_name,
            session_candidates,
            client_conversation_id or None,
        )
        ...
        try:
            for attempt in range(max_retries):
                ...
                try:
                    async with self._schedule_lock:
                        for candidate_session_id in session_candidates:
                        ...
                        if target.session_id is not None:
                            ...
                            if client_conversation_id:
                                self._session_cache.bind_client_conversation(
                                    type_name,
                                    client_conversation_id,
                                    target.session_id,
                                )
                    ...
                    if not content.strip() and (
                        req.attachment_files_last_user
                        or req.attachment_files_all_users
                    ):
                    ...
                    if session_id is None:
                        ...
                        async with self._schedule_lock:
                            account_id = self._pool.account_id(
                                target.group, target.account
                            )
                    ...
                    # 当前 user 有新附件时优先只发本轮附件，避免重建会话时把历史图片和当前图片
                    # 一起挂到同一条站点消息上，导致视觉模型参考错图。
                    attachments, attachment_source = _select_attachments_for_request(
                        req,
                        full_history=target.full_history,
                    )
                    logger.info(
                        "[chat] attachments selected count=%d source=%s last_user=%d all_users=%d full_history=%s",
                        len(attachments),
                        attachment_source,
                        len(req.attachment_files_last_user),
                        len(req.attachment_files_all_users),
                        target.full_history,
                    )
                    ...
                    async for chunk in stream:
                        if chunk:
                            saw_content = True
                        yield _CompletionChunk(content=chunk, account_id=account_id)
                    if not saw_content:
                        logger.warning(
                            "[chat] empty upstream stream, retrying: type=%s session_id=%s attempt=%d/%d",
                            type_name,
                            session_id,
                            attempt + 1,
                            max_retries,
                        ...
                        if attempt == max_retries - 1:
                            raise RuntimeError(
                                f"上游模型连续 {max_retries} 次返回空响应，请稍后再试"
                            )
                        session_candidates = []
                        continue
                    yield _CompletionChunk(
                        content=session_id_suffix(session_id),
                    ...
                    if target is not None:
                        async with self._schedule_lock:
                            ...
                            self._browser_manager.release_tab(
                                target.proxy_key, type_name
                            )
                            await self._reconcile_tabs_locked()
        finally:
            if client_conversation_claimed and client_conversation_key is not None:
                async with self._schedule_lock:
                    self._busy_client_conversations.discard(client_conversation_key)
    ...
    async def stream_openai_events(
        ...
        try:
            async for chunk in self._stream_completion(type_name, req):
                ...
                yield OpenAIStreamEvent(type="content_delta", content=chunk.content)
            ...
            try:
                usage_model = req.model or ""
                plugin = PluginRegistry.get(type_name)
                if plugin is not None:
                    usage_model = plugin.resolve_usage_model(req.model or "")
                visible_output_text = strip_session_id_suffix(output_text)
                record_usage(
                    UsageRecord(
                        provider=type_name,
                        ...
                        output_tokens=estimate_tokens(visible_output_text),
                        account_id=account_id,
                        error=error[:500],
                    )
                )
            except Exception:
                logger.exception("[usage] 记录请求用量失败")
