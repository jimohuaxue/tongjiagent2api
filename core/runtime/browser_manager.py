...
def _extract_ws_url_from_stderr(stderr_path: Path | None) -> str | None:
    """从 Chromium stderr 日志中提取 WebSocket 调试 URL。
    Chromium 启动时会打印形如: DevTools listening on ws://127.0.0.1:PORT/devtools/browser/UUID
    """
    if stderr_path is None or not stderr_path.exists():
        return None
    try:
        content = stderr_path.read_text(encoding="utf-8", errors="replace")
        m = re.search(r"DevTools listening on (ws://\S+)", content)
        if m:
            return m.group(1)
    except Exception:
        pass
...
class BrowserManager:
    ...
    async def ensure_browser(
        ...
        # 优先使用 Chromium 打印在 stderr 中的 ws:// URL，避免 http:// 探测 /json/version 返回 400
        ws_url = _extract_ws_url_from_stderr(stderr_path)
        endpoint = ws_url or f"http://127.0.0.1:{port}"
        logger.info("CDP 连接端点: %s", endpoint)
        try:
