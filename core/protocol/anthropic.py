...
class AnthropicProtocolAdapter(ProtocolAdapter):
    ...
    def parse_request(
        ...
        return CanonicalChatRequest(
            ...
            client_conversation_id=extract_client_conversation_id(raw_body),
        )
    ...
    def render_non_stream(
        ...
        if req.tools:
            ...
            if parsed.is_tool_call:
                ...
                if not any(block.get("type") == "tool_use" for block in content):
                    content.append(
                        {
                            "type": "text",
                            "text": strip_leading_final_answer_wrapper(text)
                            + session_marker,
                        }
                    )
        ...
        rendered = strip_leading_final_answer_wrapper(text)
        if session_marker:
    ...
    async def render_stream(
            ...
            async for event in raw_stream:
                if event.type == "content_delta" and event.content:
                    ...
                    for visible_chunk in final_answer_filter.feed(chunk):
                        ...
                        for out in renderer.block_delta("text", visible_chunk):
                            yield out
        ...
        session_marker = ""
        renderer = _AnthropicTaggedRenderer(req, message_id, req.tool_schema_map())
        async for event in raw_stream:
            if event.type == "content_delta" and event.content:
                chunk = event.content
                if extract_session_id_marker(chunk) and not strip_session_id_suffix(
                ...
                if item_type == "text":
                    ...
                    if source_type == "base64":
                        blocks.append(
                            CanonicalContentBlock(
                                type="document",
                                mime_type=str(source.get("media_type") or "text/plain"),
                                data=str(source.get("data") or ""),
                        ...
                        text_content = str(source.get("text") or "")
                        data_b64 = _b64.b64encode(text_content.encode()).decode()
                        blocks.append(
                            CanonicalContentBlock(
                                type="document",
...
class _AnthropicTaggedRenderer:
    ...
    def render(self, event: TaggedStreamEvent) -> list[str]:
        ...
        if event.type == "message_stop":
            ...
            if stop_reason == "tool_use" and not self._emitted_tool_call:
                stop_reason = "end_turn"
            return self.message_stop(stop_reason)
        if event.type == "error":
    ...
    def tool_call(self, name: str, arguments: dict[str, Any]) -> list[str]:
        ...
        normalized_arguments = normalize_tool_call_arguments(
            name,
            arguments,
            self._tool_schemas,
        )
        ...
        args_json = json.dumps(normalized_arguments, ensure_ascii=False)
        if args_json:
            out.append(
                self._event(
                    {
